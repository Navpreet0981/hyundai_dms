...
export default function TestDrives() {
  ...
  const createTestDrive = () => {
    ...
    api.post("/testdrives", {
      ...
      .then(() => { alert("Test Drive Scheduled"); setDate(""); loadTestDrives(); })
      .catch(err => console.log(err));
  };
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm overflow-x-auto">
            <table className="w-full text-sm min-w-[800px]">
              ...
              <tbody>
                {testDrives.map(t => (
                  <tr key={t.testDriveId} className="border-t border-gray-200 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800">
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {t.status === "COMPLETED" && (
                          <button className="px-3 py-1 text-xs bg-purple-600 hover:bg-purple-700 text-white rounded" onClick={() => createBooking(t)}>Create Booking</button>
                        )}
                      </div>
                    </td>
                  </tr>
