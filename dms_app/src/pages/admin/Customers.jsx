...
export default function Customers() {
  ...
  // Debounce search input — prevents API call on every keystroke, waits 400ms
  useEffect(() => {
    const t = setTimeout(() => { setDebouncedSearch(search); setPage(0); }, 400);
    return () => clearTimeout(t);
  }, [search, setPage]);

  // Fetch paginated customers — admin sees all, backend scopes by role automatically
  const { data, isLoading: loading } = useCustomersPaged(page, size, debouncedSearch, sort);
  const customers = data?.content ?? [];

  // Keep pagination hook in sync with total pages returned by the backend
  useEffect(() => { if (data?.totalPages !== undefined) setTotalPages(data.totalPages); }, [data?.totalPages, setTotalPages]);

  // Three-state sort toggle per column: none → asc → desc → none
      ...
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Sort dropdown — value is passed as Spring Sort param to backend */}
